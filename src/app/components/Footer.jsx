...
export default function Footer() {
  // We no longer need useState or a submit handler!

  // This function will be called when the button is clicked.
  // It checks if the Mailchimp script has loaded and then calls its function to show the form.
  const openMailchimpPopup = () => {
    if (window.dojo) {
      window.dojo.MCE.showForm();
    } else {
      // As a fallback, you could open the signup page in a new tab
      // You can find this URL in Mailchimp under Signup Forms -> Form Builder
      // window.open('YOUR_MAILCHIMP_HOSTED_FORM_URL', '_blank');
      alert('Could not load the subscription form. Please try again later!');
    }
  };
  ...
  return (
    <footer className="bg-nautical text-antique relative mt-20 border-t border-antique min-h-[400px] md:min-h-[500px] lg:min-h-[600px]">
      {/* ... your background images and overlay divs ... */}
      <div className="absolute inset-0 z-0 opacity-5"
           style={{
      ...
      <div className="container mx-auto px-4 py-12 relative z-20">
        ...
        {/* ... The rest of your footer content (Quick Links, Contact, Social, etc.) remains the same ... */}
